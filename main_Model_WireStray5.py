...
test_data[:,0] = fov/2
test_data[:,1] = np.linspace(1, fov, 200) # No idea what the *2 does. Leftover from 8/4/2024
test_data[:,2] = fovz/2

# Construct a 1000-point Bezier curve from the provided data
bez_curve = bez.BezierSegment(F3)
params = np.arange(0, 1, 0.001)
bez_points_test = np.zeros((len(params), 3))
for t in range(len(params)):
    bez_points_test[t][:] = np.asarray(bez_curve.point_at_t(params[t]))
...
Bx_test_wire, By_test_wire, Bz_test_wire = calculate_magnetic_field(bez_points_test, current_pos, voxel_size, bounds, im_size, nslices)
